...
func postInitialize(w http.ResponseWriter, r *http.Request) {
	...
	dbx.Select(&allCategories, "SELECT * FROM `categories`")
	log.Printf("allCategories: %v\n", allCategories)
...
func postShip(w http.ResponseWriter, r *http.Request) {
	...
	item := Item{}
	err = tx.Get(&item, "SELECT * FROM `items` WHERE `id` = ? FOR UPDATE /* postShip */", itemID)
	if err == sql.ErrNoRows {
		outputErrorMsg(w, http.StatusNotFound, "item not found")
		tx.Rollback()
		return
...
func postShipDone(w http.ResponseWriter, r *http.Request) {
	...
	item := Item{}
	err = tx.Get(&item, "SELECT * FROM `items` WHERE `id` = ? FOR UPDATE /* postShipDone */", itemID)
	if err == sql.ErrNoRows {
...
func postComplete(w http.ResponseWriter, r *http.Request) {
	...
	go APIShipmentStatus(getShipmentServiceURL(), &APIShipmentStatusReq{
		ReserveID: shipping.ReserveID,
	})
